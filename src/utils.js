...
export function getNormalizedAndLocalizedLanguageDefinitions(editor) {
    const languageDefs = editor.config.get('codeBlock.languageLabel');
    // {c:'C'} => [{language:'c', label:'C'}]
    return Object.keys(languageDefs).map(key => ({ language: key, label: languageDefs[key] }));
}
...
export function domCodeToModel(dom, writer) {
    ...
    const models = Array.prototype.map.call(dom.children[0].childNodes, node => {
